...
resource "aws_ebs_volume" "vol10" {
  count             = var.enabled_ebs_volume10 ? 1 : 0
  size              = var.ebs_volume10_size
  type              = var.volume_type
  iops              = contains(["gp3", "io1", "io2"], var.volume_type) ? var.ebs_volume10_iops : null
  throughput        = contains(["gp3"], var.volume_type) ? var.ebs_volume10_throughput : null
  snapshot_id       = var.snapshot_id_volume10
  availability_zone = aws_instance.this.*.availability_zone[0]
  tags                        = var.tags
  lifecycle {
    ignore_changes = [availability_zone]
...
resource "aws_volume_attachment" "attachment10" {
  count       = var.enabled_ebs_volume10 ? 1 : 0
  device_name = "/dev/sdp"
  volume_id   = aws_ebs_volume.vol10.*.id[0]
  instance_id = aws_instance.this.*.id[0]
  lifecycle {
    ignore_changes = [instance_id,volume_id]
  }
...
resource "aws_ebs_volume" "vol11" {
  count             = var.enabled_ebs_volume11 ? 1 : 0
  size              = var.ebs_volume11_size
  type              = var.volume_type
  iops              = contains(["gp3", "io1", "io2"], var.volume_type) ? var.ebs_volume11_iops : null
  throughput        = contains(["gp3"], var.volume_type) ? var.ebs_volume11_throughput : null
  snapshot_id       = var.snapshot_id_volume11
  availability_zone = aws_instance.this.*.availability_zone[0]
  tags                        = var.tags
  lifecycle {
    ignore_changes = [availability_zone]
...
resource "aws_volume_attachment" "attachment11" {
  count       = var.enabled_ebs_volume11 ? 1 : 0
  device_name = "/dev/sdq"
  volume_id   = aws_ebs_volume.vol11.*.id[0]
  instance_id = aws_instance.this.*.id[0]
  lifecycle {
    ignore_changes = [instance_id,volume_id]
  }
...
resource "aws_ebs_volume" "vol12" {
  count             = var.enabled_ebs_volume12 ? 1 : 0
  size              = var.ebs_volume12_size
  type              = var.volume_type
  iops              = contains(["gp3", "io1", "io2"], var.volume_type) ? var.ebs_volume12_iops : null
  throughput        = contains(["gp3"], var.volume_type) ? var.ebs_volume12_throughput : null
  snapshot_id       = var.snapshot_id_volume12
  availability_zone = aws_instance.this.*.availability_zone[0]
  tags                        = var.tags
  lifecycle {
    ignore_changes = [availability_zone]
...
resource "aws_volume_attachment" "attachment12" {
  count       = var.enabled_ebs_volume12 ? 1 : 0
  device_name = "/dev/sdr"
  volume_id   = aws_ebs_volume.vol12.*.id[0]
  instance_id = aws_instance.this.*.id[0]
  lifecycle {
    ignore_changes = [instance_id,volume_id]
  }
